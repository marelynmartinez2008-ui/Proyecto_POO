🐾 Sistema de Gestión - Centro de Adopción de Perritos

Este es un sistema web desarrollado con **Python** y **Flask** para gestionar la adopción de mascotas. Permite a los usuarios ver un catálogo de perros disponibles y a los administradores gestionar el historial de adopciones y añadir nuevos integrantes al sistema.

🚀 Características

- **Catálogo Dinámico:** Visualización de perritos con fotos, edad y raza.
- **Sistema de Usuarios:** Roles diferenciados para Adoptantes y Administradores.
- **Panel de Administrador:** - Visualización de historial de adopciones.
  - Formulario para agregar nuevos perros al catálogo.
- **Transacciones Seguras:** Registro de adopciones utilizando transacciones SQL (ACID) para garantizar la integridad de los datos.
- **Base de Datos:** Implementado con MariaDB/MySQL.

Tecnologías Utilizadas

* **Backend:** Python 3 + Flask
* **Base de Datos:** MariaDB / MySQL
* **Frontend:** HTML5, CSS3 (Google Fonts - Poppins)
* **Librerías:** `mariadb`, `flask`

Requisitos Previos

Antes de ejecutar el proyecto, asegúrate de tener instalado:
- Python 3.12+
- MariaDB Server
- Driver de MariaDB para Python: `pip install mariadb`

Instalación y Configuración

1. **Clonar el repositorio:**
   ```bash
   git clone [https://github.com/tu-usuario/tu-repositorio.git](https://github.com/tu-usuario/tu-repositorio.git)
   cd Centro_Adopcion
<img src="https://github.com/marelynmartinez2008-ui/Proyecto_POO/blob/main/Captura%20desde%202026-04-09%2007-53-55.png?raw=true" width="600" alt="Catalogo de perritos">
<img src="https://github.com/marelynmartinez2008-ui/Proyecto_POO/blob/main/Captura%20desde%202026-04-09%2007-59-34.png?raw=true" width="600" alt="Catalogo de perritos">
<img src="https://github.com/marelynmartinez2008-ui/Proyecto_POO/blob/main/Captura%20desde%202026-04-09%2007-54-30.png?raw=true" width="600" alt="Catalogo de perritos">
<img src="https://github.com/marelynmartinez2008-ui/Proyecto_POO/blob/main/Captura%20desde%202026-04-09%2007-55-09.png?raw=true" width="600" alt="Catalogo de perritos">
<img src="https://github.com/marelynmartinez2008-ui/Proyecto_POO/blob/main/Captura%20desde%202026-04-09%2007-55-19.png?raw=true" width="600" alt="Catalogo de perritos">
